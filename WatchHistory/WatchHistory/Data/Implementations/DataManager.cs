...
#pragma warning restore IDE1006 // Naming Styles
    ...
    public void AddWatched(FileEntry entry, string userName)
        => this.AddWatched(entry, userName, DateTime.UtcNow);
    ...
    private void LoadSettings()
    {
        ...
        this.HideDeleted = defaultValues.HideDeleted;
    }
    ...
    private bool IsProtected(FileEntry entry)
        => entry.FullName.EndsWith(MIHC.DvdProfilerFileExtension) && entry.VideoLengthSpecified;
    ...
    private IEnumerable<Task<IEnumerable<string>>> GetActualFiles(string folder)
        => _fileExtensions.Select(ext => this.GetActualFiles(folder, ext));

    private Task<IEnumerable<string>> GetActualFiles(string folder, string fileExtension)
        => Task.Run(() => this.GetFiles(folder, fileExtension));
    ...
    private IEnumerable<string> GetFiles(string rootFolder, string fileExtension)
        => _ioServices.Folder.Exists(rootFolder)
            ? _ioServices.Folder.GetFileNames(rootFolder, "*." + fileExtension, SearchOption.AllDirectories)
            : [];
    ...
    private void RemoveObsoletesFiles(IEnumerable<string> actualFiles)
    {
        lock (_filesLock)
        {
            ...
            var actualFileKeys = actualFiles.Select(FileEntry.GetKey).ToList();
    ...
    private int CompareEntries(FileEntry left, FileEntry right)
    {
        var compare = 0;
