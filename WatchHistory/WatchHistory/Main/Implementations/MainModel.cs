...
internal sealed class MainModel : ModelBase, IMainModel
{
    ...
    public IEnumerable<FileEntry> GetFiles()
    {
        ...
        var existingFiles = _dataManager.HideDeleted
            ? [.. allFiles.Where(entry => entry.FileExists)]
            : allFiles;

        var notIgnoredFiles = existingFiles.Except(existingFiles.Where(this.UserIgnores));

        var filteredFiles = notIgnoredFiles.Where(this.ContainsFilter).ToList();

        var unwatchedFiles = this.IgnoreWatched
            ? [.. filteredFiles.Except(filteredFiles.Where(this.UserHasWatched))]
            : filteredFiles;
    ...
    private bool UserHasWatched(FileEntry file) 
        => file.Users?.HasItemsWhere(this.UserHasWatched) == true;

    private bool UserHasWatched(Data.User user) 
        => (this.IsUser(user)) && (user.Watches?.HasItems() == true);
