...
internal sealed class SettingsViewModel : ISettingsViewModel
{
    ...
    public SettingsViewModel(IDataManager dataManager, IUIServices uiServices)
    {
        ...
        this.Users = [.. _dataManager.Users.Select(item => new SettingsListBoxItemViewModel(item))];

        this.RootFolders = [.. _dataManager.RootFolders];

        this.FileExtensions = [.. _dataManager.FileExtensions.Select(item => new SettingsListBoxItemViewModel(item))];
    ...
    {
        get => _dataManager.HideDeleted;
        set
        {
            if (value != _dataManager.HideDeleted)
            {
                _dataManager.HideDeleted = value;

                this.RaisePropertyChanged(nameof(this.HideDeleted));
            }
        }
    }
    ...
    private void AddUser()
        => this.Users.Add(new SettingsListBoxItemViewModel(string.Empty));
